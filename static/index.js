...
$(function () {
    $(":radio").click(function () {
        ...
        window.location = `${location.origin}?map=${current}`
    });
    ...
    socket.on('update', function (snapshot) {
        ...
        if (Array.isArray(players)) {
            players.forEach(item => {
                const mapname = Utils.getParameterByName('map');

                //positonConfig 为全局变量，在html的引用中
                const {pos_x, pos_y} = positionConfig[mapname];
                item.x = item.x + Math.abs(pos_x);
                item.y = pos_y - item.y;
    ...
    var maps = [
        'de_cache_radar.png', 'de_cbble_radar.png', 'de_dust2_radar.png',
        'de_inferno_radar.png', 'de_mirage_radar.png', 'de_nuke_radar.png',
        'de_overpass_radar.png', 'de_train_radar.png'];


    var mapParameter = Utils.getParameterByName('map');
    if (mapParameter) {
        $("input[name='map'][value='" + mapParameter + "']").attr("checked", true);
    }


    radar.setMap(mapParameter ? `${mapParameter}_radar.png` : maps[0]);
    // if (mapParameter === '1') {
    //     radar.setMap(maps[0]);
